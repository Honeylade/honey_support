...
TAGS_TO_INCLUDE = ["football accessories", "football", "rugby", "entertainment", "themed gifts", "Honeylade", "Honey", "sports gifts", "fan accessories", "novelty gifts", "sports fans"]
...
def build_product(p):
    ...
    price = calc_price(float(p.get("costprice", 0)), float(p.get("weight", 0)))
...
def load_xml():
    ...
    try:
        ...
        print(f"🔎 Found {len(items)} items (full parse).")
    except ET.ParseError as e:
        print(f"⚠️ XML ParseError: {e}")
