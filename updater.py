...
def build_product(p):
    ...
    # Get stock quantity from the XML data
    ...
    if sizes:
        for s in sizes:
            variants.append({
                ...
                "price": price,
                "sku": p.get("sku"),
                "inventory_quantity": stock_qty,  # Include stock quantity
...
def get_updated_products():
    url = f"https://{SHOP_URL}/admin/api/{API_VERSION}/products.json"
    try:
        res = shopify_get(url, params={"updated_at_min": "2023-08-27T00:00:00Z"})  # Update this to current timestamp
...
def sync_product(p, counters, resync_quantity_counter):
    ...
    if existing:
        ...
        stock_qty = product_payload['variants'][0]['inventory_quantity']  # Use stock_qty directly
 
        # Check for changes
        if existing_price != new_price or existing_inventory_quantity != stock_qty:
            needs_update = True
        ...
        if needs_update:
            ...
            response = shopify_put(url, {
                "product": {
                    **product_payload,
                    "variants": [{
                        **product_payload["variants"][0],
                        "inventory_quantity": stock_qty  # Use stock_qty directly here
                    }]
                }
            })
            ...
            if response:
                ...
                counters['updated'] += 1
                resync_quantity_counter += stock_qty
            else:
    ...
    while True:
        ...
        try:
            ...
            for product in updated_products:
                print(f"Syncing product: {product.get('title')}")
