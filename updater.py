...
# Minimum number of supplier products required before syncing.
# This protects Shopify from a partial/empty feed after XML recovery.
MIN_FEED_PRODUCTS = int(os.getenv("MIN_FEED_PRODUCTS", "100"))
...
def _posts_to_products(items) -> List[Dict[str, Any]]:
    """Convert parsed <post> elements into the supplier product format."""
    products = []
    ...
    for item in items:
        ...
        for child in item:
            ...
            if key:
                data[key] = clean_text(child.text)
        if data:
...
def _recover_xml_posts(raw_xml: bytes) -> List[Dict[str, Any]]:
    """
    Recover complete <post> records from a malformed supplier XML feed.

    The supplier feed is expected to contain flat <post> records. If the
    document as a whole is malformed, each individual post is parsed on its
    own so one broken record does not prevent the remaining feed from being
    processed. Records that cannot be parsed are skipped and reported.
    """
    post_pattern = re.compile(
        rb"<(?:(?:[A-Za-z_][\w.-]*):)?post\b[^>]*>.*?</(?:(?:[A-Za-z_][\w.-]*):)?post\s*>",
        re.IGNORECASE | re.DOTALL,
    )

    fragments = post_pattern.findall(raw_xml)
    if not fragments:
        return []
    ...
    for fragment in fragments:
        try:
            item = ET.fromstring(fragment)
            converted = _posts_to_products([item])
            if converted:
                recovered.extend(converted)
            else:
                skipped += 1
        except ET.ParseError:
            skipped += 1

    print(f"🛠️ XML recovery found {len(fragments)} <post> records")
    print(f"   ✅ Recovered: {len(recovered)}")
    if skipped:
        print(f"   ⚠️ Skipped malformed records: {skipped}")
...
def load_xml() -> List[Dict[str, Any]]:
    ...
    # First try the normal XML parser. This is the preferred path because it
    # validates the entire supplier document before anything is synced.
    try:
        root = ET.fromstring(raw_xml)
        items = [
            element
            for element in root.iter()
            if _xml_local_name(element.tag) == "post"
        ]
        products = _posts_to_products(items)
        print(f"🔎 Found {len(products)} supplier items")

    except ET.ParseError as exc:
        print(f"⚠️ Supplier XML is malformed: {exc}")
        print("🛠️ Attempting safe record-level XML recovery...")
        products = _recover_xml_posts(raw_xml)

        if not products:
            raise RuntimeError(
                "Supplier XML could not be parsed or safely recovered. "
                "No <post> records were recovered; Shopify sync stopped."
            )

    # Never continue with a suspiciously small recovered feed. This prevents
    # a damaged supplier feed from making thousands of valid Shopify products
    # appear missing in downstream cleanup/sync logic.
    if len(products) < MIN_FEED_PRODUCTS:
        raise RuntimeError(
            f"Supplier feed safety check failed: only {len(products)} products "
            f"were loaded, below MIN_FEED_PRODUCTS={MIN_FEED_PRODUCTS}. "
            "Shopify sync stopped to avoid a partial feed update."
        )

    selected_items = (
        products[:LIMIT] if LIMIT is not None else products
    )

    if LIMIT is not None:
        print(f"🎯 Applying LIMIT={LIMIT}; syncing {len(selected_items)} items")
    ...
    if source["sizes"] and (created or CREATE_MISSING_SIZE_VARIANTS_FOR_EXISTING_PRODUCTS):
        try:
            ...
            if created_variants:
                ...
                if refreshed:
                    product = refreshed
                    add_product_to_catalog(catalog, refreshed)
        except Exception as exc:
