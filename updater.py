...
def shopify_post(url, data, retries=3):
    for attempt in range(retries):
        try:
            r = requests.post(url, headers=HEADERS, json=data, timeout=10)  # Set timeout to 10 seconds
            if r.status_code in [200, 201]:
                return r.json()
            else:
                print(f"❌ POST ERROR: {r.status_code} {r.text}")
                if r.status_code == 504:  # Gateway timeout
...
def shopify_put(url, data, retries=3):
    for attempt in range(retries):
        try:
            r = requests.put(url, headers=HEADERS, json=data, timeout=10)  # Set timeout to 10 seconds
            if r.status_code in [200, 201]:
                return r.json()
            else:
...
def sync_product(p):
    ...
    for attempt in range(3):  # Retry up to 3 times
        try:
            ...
            print(f"⏳ Timeout occurred for {title}, retrying...")
            time.sleep(2)  # Wait before retrying
...
def run_sync():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:  # Increased to 8 workers
