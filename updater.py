...
def sync_product(p, counters, resync_quantity_counter):
    ...
    if existing:
        ...
        # Check for changes in price or inventory quantity
        if existing_price != new_price:
            needs_update = True
            print(f"Price change detected for {product_payload['title']}: {existing_price} -> {new_price}")
 
        if existing_inventory_quantity != new_inventory_quantity:
            needs_update = True
            print(f"Inventory change detected for {product_payload['title']}: {existing_inventory_quantity} -> {new_inventory_quantity}")
 
        if needs_update:
            url = f"https://{SHOP_URL}/admin/api/{API_VERSION}/products/{existing['id']}.json"
            response = shopify_put(url, {
                "product": {
                    "id": existing['id'],  # Include existing product ID
                    "variants": [{
                        "id": existing['variants'][0]['id'],  # Ensure the variant ID is included
                        "price": new_price,
                        "inventory_quantity": new_inventory_quantity  # Update inventory quantity
                    }]
                }
            })
            ...
            if response and "product" in response:
                ...
                print(f"❌ Failed to update: {product_payload['title']} - Response: {response}")
        else:
        ...
        if response and "product" in response:
            ...
            print(f"❌ Failed to create: {product_payload['title']} - Response: {response}")
