        ...
        for p in res.get("products", []):
            for v in p.get("variants", []):
                if (sku and str(v.get("sku")) == str(sku)) or (barcode and v.get("barcode") and v.get("barcode") == barcode):
                    return p
    url = f"https://{SHOP_URL}/admin/api/{API_VERSION}/products.json"
    params = {"limit": 250}  # Fetch up to 250 products at once
...
def load_xml():
    ...
    try:
        ...
        print(f"⚠️ XML ParseError on full parse: {e}. Check the following raw XML snippet for context:\n{raw[:500]}")  # Show first 500 chars
    except Exception as e:
...
def process_product(p):
    ...
    if existing:
        # Compare existing variants with new variants
        existing_variants = existing.get("variants", [])
        new_variants = product_payload.get("variants", [])
        
        if existing_variants == new_variants:  # Check if existing and new variants match
            print(f"✅ Product already exists and is up-to-date: {title} (ID: {existing['id']})")
            return "exists"  # Indicate that the product exists without changes
