...
XML_URL = os.getenv(
    "XML_URL",
    "https://customerfeeds.taylorsmerchandise.com/v1/products/All-Products-Daily-Feed98478.xml"
).strip()
...
# Recommended for approximately 5,000 products:

# 4 workers.

# Increase to 5-6 only after confirming Shopify is handling

# the traffic without 429 responses.
...
# If only one active location exists, the script can select it
...
def resolve_location_id() -> str:
    ...
    raise RuntimeError(
        ...
        "Set LOCATION_ID to the location that "
        "should receive supplier stock."
    )
...
def build_source_product(p: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sizes = [
        size.strip()
        for size in re.split(
            r"[|,]+",
            clean_text(p.get("sizeattribute"))
        )
        if size.strip()
    ]
...
def set_inventory(inventory_item_id: str, location_id: str, quantity: int) -> None:
    ...
    #
    # None tells Shopify to skip the compare-and-swap
    # quantity check.
    # --------------------------------------------------------
